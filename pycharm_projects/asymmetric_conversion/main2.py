...
ase_input_data = read("inputs/"+structure_name+".cif", store_tags=False)
...
if find_primitive:
    ...
    lattice, positions, numbers = spglib.find_primitive(spg_input, symprec=1e-3)
    ...
    write(output_dir+'/'+structure_name+'_primtive_cell.xyz', ase_primitive_cell)
else:
...
# Reconstruct from primitive cell
aei_cell = ase_spacegroup.crystal(ase_primitive_cell, spacegroup=dataset['number'])
write(output_dir + '/reconstructed_from_prim_' + structure_name + '_cell.xyz', aei_cell)
...
write(output_dir + '/asym_' + structure_name + '_cell.xyz', ase_asymmetric_cell)
...
if test_sym_ops:
    ...
    #ase_view(aei_cell)
    write(output_dir+'/reconstructed_'+structure_name+'_cell.xyz', aei_cell)
