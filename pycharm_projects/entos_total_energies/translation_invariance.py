...
def mos2_trans_invariance_strings(named_result: str, arb_shift: float) \
        -> typing.Tuple[str]:
    """
        MoS2
    ...
    input_string = named_result + """ := xtb(
          structure( fractional=[[Mo, {pos0:s}],
                                 [S,  {pos1:s}],
                                 [S,  {pos2:s}]] 
                 lattice(  a=6.856 angstrom
                           alpha = 26.94 degree
...
def mos2_input_str() -> str:
    named_result = 'mos2'
    (input1, input2) = mos2_trans_invariance_strings(named_result, arb_shift=0.53)
    assert_variables = {"n_iter":24, "energy":-9.505776937}
    assert_string = get_assert_string(named_result, assert_variables)
    return input1 + '\n' + assert_string + input2 + '\n' + assert_string
...
if generate_app_test_inputs:
    #TODO(Alex) Get 13 entries in this list
