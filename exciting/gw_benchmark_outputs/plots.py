...
def parse_gw_results(root:str, settings:dict) -> dict:
    ...
    max_energy_cutoff = settings['max_energy_cutoff']
    ...
    for i, l_max in enumerate(l_max_values):
        ...
        for ienergy, energy in enumerate(max_energy_cutoff):
...
def process_basis_numbers(l_max_values, delta_E_qp, max_energy_cutoff):
    """
    ...
    max_energy = 160
    ienergy = max_energy_cutoff.index(max_energy)
    prior_lmax = [3, 2]
    ...
    # For (5,4), change in energy for each energy parameter
    lo_54 = 2
    print("For basis (Zr,O) = (5,4)")
    for ie, energy in enumerate(max_energy_cutoff[1:], start=1):
        change_in_delta_E_qp = (delta_E_qp[ie, lo_54] - delta_E_qp[ie-1, lo_54]) * ha_to_mev
        print("Change in Delta E_QP from max energy param ", max_energy_cutoff[ie-1], "to ", energy, ":", change_in_delta_E_qp, "(meV)")
...
def gw_basis_convergence(root:str):
    ...
    max_energy_cutoff = [60, 80, 100, 120, 140, 160, 180, 200]
    ...
    process_basis_numbers(l_max_values, delta_E_qp, max_energy_cutoff)
    ...
    # TODO(Alex)
    # Will need a separate parsing and plot for D([('zr', 6), ('o', 5)])]
    # as I only intend to go to 120 Ha.
