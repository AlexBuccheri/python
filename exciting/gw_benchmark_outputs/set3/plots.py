...
def process_basis_numbers(l_max_values, delta_E_qp, max_energy_exts:list):
    """
...
def gw_basis_convergence(root:str):
    ...
    # Set 2 results
    data2 = parse_gw_results(root + "/A1_var_cutoff",
                                   {'rgkmax': 7,
                                    'l_max_values': [D([('zr', 4), ('o', 3)])],
                                    'n_img_freq': 32,
                                    'q_grid': [2, 2, 2],
                                    'n_empty_ext': [1000],
                                    'max_energy_cutoffs': [90, 160, 230, 300]})
    delta_E_qp2 = data2['delta_E_qp']

    data3 = parse_gw_results(root + "/A1_set3",
                                   {'rgkmax': 7,
                                    'l_max_values': [D([('zr', 4), ('o', 3)])],
                                    'n_img_freq': 32,
                                    'q_grid': [2, 2, 2],
                                    'n_empty_ext': [1000],
                                    'max_energy_cutoffs': ['i0', 'i1', 'i2', 'i4', 'i5', 'i6', 'i7']})   # 'i3']
    # i7. GW VBT and CBB get fucked up

    delta_E_qp3 = data3['delta_E_qp']

    print(delta_E_qp2 * ha_to_mev)
    print(delta_E_qp3 * ha_to_mev)

    # Make QP vs basis size (total basis size)
